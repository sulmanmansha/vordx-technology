"use client"; // Moved this to the top of the file

import Image from "next/image";
import React from "react";
import { motion } from "framer-motion";

const UI = () => {
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-[14px]">
      {/* Card 1 (wide) - Height: 480px */}
      <motion.div
        className="md:col-span-2 relative h-[480px] rounded-[20px] overflow-hidden group"
        initial={{ opacity: 0, y: 30 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.6, delay: 0.3 }}
      >
        <Image
          src="/images/ui1.png"
          fill
          alt="team"
          className="object-cover w-full h-full"
        />
        <div className="absolute inset-0 bg-black/40 translate-y-full group-hover:translate-y-0 transition-all duration-700"></div>
        <div className="absolute inset-0 flex flex-col justify-between p-4 sm:p-8 text-white opacity-0 group-hover:opacity-100 transition-opacity duration-700 group-hover:delay-500">
          <h3 className="md:text-[36px] text-[26px] leading-9 md:leading-[44px] text-start pro-medium">
            An intelligent chatbot designed to simplify communication and
            enhance customer interactions.
          </h3>
          <div className="flex items-center justify-between text-sm flex-wrap">
            <a
              href="#"
              className="flex pro-normal items-center gap-2 text-[14px]"
            >
              View Case Study <span className="pt-1 text-[21px]">→</span>
            </a>
            <span className="text-[14px] pro-normal text-[#FFFFFFCC] leading-5">
              Web App UI/UX, Branding
            </span>
          </div>
        </div>
      </motion.div>

      {/* Card 2 (normal) - Height: 480px */}
      <motion.div
        className="relative h-[480px] rounded-[20px] overflow-hidden group"
        initial={{ opacity: 0, y: 30 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.6, delay: 0.6 }}
      >
        <Image
          src="/images/ui2.png"
          fill
          alt="team"
          className="object-fill w-full h-full"
        />
        <div className="absolute inset-0 bg-black/40 translate-y-full group-hover:translate-y-0 transition-all duration-700"></div>
        <div className="absolute inset-0 flex flex-col justify-between pt-4 pr-[18px] pb-6 pl-4 sm:pt-6 sm:pr-[22px] sm:pb-9 sm:pl-6 text-white opacity-0 group-hover:opacity-100 transition-opacity duration-700 group-hover:delay-500">
          <h3 className="md:text-[36px] text-[26px] leading-9 md:leading-[44px] text-left pro-medium">
            Personalized tutoring made easy connect, learn, and grow anytime,
            anywhere.
          </h3>
          <div className="flex items-center justify-between text-sm flex-wrap">
            <a
              href="#"
              className="flex pro-normal items-center gap-2 text-[14px]"
            >
              View Case Study <span className="pt-1 text-[21px]">→</span>
            </a>
            <span className="text-[14px] pro-normal text-[#FFFFFFCC] leading-5">
              Mobile App, UI/UX, Branding
            </span>
          </div>
        </div>
      </motion.div>

      {/* Card 3 (full) - Height: 520px */}
      <motion.div
        className="md:col-span-3 relative h-[500px] rounded-[20px] overflow-hidden group"
        initial={{ opacity: 0, y: 30 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.6, delay: 0.9 }}
      >
        <Image
          src="/images/ui3.png"
          fill
          alt="team"
          className="object-fill w-full h-full"
        />
        <div className="absolute inset-0 bg-black/40 translate-y-full group-hover:translate-y-0 transition-all duration-700"></div>
        <div className="absolute inset-0 flex flex-col justify-between p-4 sm:p-8 text-white opacity-0 group-hover:opacity-100 transition-opacity duration-700 group-hover:delay-500">
          <h3 className="md:text-[36px] text-[26px] leading-9 md:leading-[44px] text-start pro-medium">
            Smart analytics and observability dashboard to <br /> monitor system
            health, detect anomalies, and ensure compliance.
          </h3>
          <div className="flex items-center justify-between text-sm flex-wrap">
            <a
              href="#"
              className="flex pro-normal items-center gap-2 text-[14px]"
            >
              View Case Study <span className="pt-1 text-[21px]">→</span>
            </a>
            <span className="text-[14px] pro-normal text-[#FFFFFFCC] leading-5">
              Web App, UI/UX, Branding
            </span>
          </div>
        </div>
      </motion.div>

      {/* Card 4 (normal) - Height: 480px */}
      <motion.div
        className="relative h-[480px] rounded-[20px] overflow-hidden group"
        initial={{ opacity: 0, y: 30 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.6, delay: 1.2 }}
      >
        <Image
          src="/images/ui4.png"
          fill
          alt="team"
          className="object-cover w-full h-full"
        />
        <div className="absolute inset-0 bg-black/40 translate-y-full group-hover:translate-y-0 transition-all duration-700"></div>
        <div className="absolute inset-0 flex flex-col justify-between pt-4 pr-[18px] pb-6 pl-4 sm:pt-6 sm:pr-[22px] sm:pb-9 sm:pl-6 text-white opacity-0 group-hover:opacity-100 transition-opacity duration-700 group-hover:delay-500">
          <h3 className="md:text-[36px] text-[26px] leading-9 md:leading-[44px] text-left pro-medium">
            A modern matchmaking platform built around faith and values.
          </h3>
          <div className="flex items-center justify-between text-sm flex-wrap">
            <a
              href="#"
              className="flex pro-normal items-center gap-2 text-[14px]"
            >
              View Case Study <span className="pt-1 text-[21px]">→</span>
            </a>
            <span className="text-[14px] pro-normal text-[#FFFFFFCC] leading-5">
              Mobile App, UI/UX, Branding
            </span>
          </div>
        </div>
      </motion.div>

      {/* Card 5 (wide) - Height: 480px */}
      <motion.div
        className="md:col-span-2 relative h-[480px] rounded-[20px] overflow-hidden group"
        initial={{ opacity: 0, y: 30 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.6, delay: 1.5 }}
      >
        <Image
          src="/images/ui5.png"
          fill
          alt="team"
          className="object-cover w-full h-full"
        />
        <div className="absolute inset-0 bg-black/40 translate-y-full group-hover:translate-y-0 transition-all duration-700"></div>
        <div className="absolute inset-0 flex flex-col justify-between p-4 sm:p-8 text-white opacity-0 group-hover:opacity-100 transition-opacity duration-700 group-hover:delay-500">
          <h3 className="md:text-[36px] text-[26px] leading-9 md:leading-[44px] text-start pro-medium">
            A powerful trading dashboard that helps users monitor, analyze, and
            copy top-performing strategies in real time.
          </h3>
          <div className="flex items-center justify-between text-sm flex-wrap">
            <a
              href="#"
              className="flex pro-normal items-center gap-2 text-[14px]"
            >
              View Case Study <span className="pt-1 text-[21px]">→</span>
            </a>
            <span className="text-[14px] pro-normal text-[#FFFFFFCC] leading-5">
              Web App, UI/UX, Branding
            </span>
          </div>
        </div>
      </motion.div>
    </div>
  );
};

export default UI;
