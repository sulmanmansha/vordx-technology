...
const Calculations = () => {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      ([entry]) => {
        // Start the count when the section is in view
        if (entry.isIntersecting) {
          setStartCount(true);
        } else {
          // Stop the count when the section is out of view
          setStartCount(false);
        }
      },
      // The animation will start when 50% of the section is visible
      { threshold: 0.5 }
    );
  ...
  const stats = [
    {
      ...
      endValue: 200,
      suffix: "+",
      duration: 3, // Duration set to a more reasonable value
      step: 10,
...
grid lg:grid-cols-2 gap-10 lg:gap-24 text-white max-[1200px]:!grid-cols-1"
      >
        ...
        {/* Right - Stats */}
        <div className="relative grid sm:grid-cols-2 gap-x-4 gap-y-4 grid-cols-1">
          {/* Horizontal Divider */}
          {/* <div
          ...
          {stats.map((stat, i) => (
            <div
              key={i}
              className="space-y-4 max-w-[260px] bg-[#14171C] rounded-lg p-3 max-[1200px]:max-w-full"
            >
                ...
                {startCount ? (
                  <CountUp
                    ...
                    formattingFn={(value) => {
                      if (stat.step) {
                        const roundedValue =
                          Math.floor(value / stat.step) * stat.step;
